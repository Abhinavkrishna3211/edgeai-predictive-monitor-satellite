...
This is the *payload* format, independent of any transport envelope: our
firmware's net_task.c publishes these same bytes as the MQTT message body
with no extra wrapper (epm/<node_id>/data).
...
Wire layout (little-endian throughout):
  ...
  doesn't recognize by length instead of failing.
...
carries opaque numbers. decode_frame() resolves channel_id -> channel name
via CHANNEL_NAME_BY_ID.
...
section, and NOT bin_count=0). bin_count=0 is the distinct "channel not part
of this node at all" case and contributes no key to bins.
"""
import struct
...
class DecodedFrame:
    ...
    `bins` is the ready-to-consume view (channel name -> spectrum bins);
    a section whose channel_id isn't in the schema, or whose bin_count is 0,
    contributes no key. `scalars`/`time_series` are decoded but keyed by raw
    wire id -- callers resolve names via schema.SCALAR_NAME_BY_ID themselves.
    """
...
# --- encode (used by tests, and by a future satellite_sim.py MQTT port) -----
...
def _decode_spectrum(out: DecodedFrame, channel_id: int, body: bytes) -> None:
    ...
    if name is None:
        # No schema mapping for this channel_id -- keep nothing under a
        # synthetic key; count it so it isn't silently dropped without trace.
