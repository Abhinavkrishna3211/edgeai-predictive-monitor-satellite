...
(Full text as provided by Abhi, 2026-07-30 — held here as the cross-phase source of truth. The planning tool tracks phase completion against this document; the AI coding assistant executes one phase at a time from a scoped prompt derived from it. See docs/PHASE_0_PROMPT.md for the current phase.)
...
- **Reference repo** (`edgeai-predictive-monitor`, the reference base station's) — a working Arduino Uno Q base station (STM32U585 MCU + Qualcomm QRB2210 MPU/Linux) with its own satellite firmware and Python gateway, built with professional embedded discipline.

Repo links:
- This repo (to be changed): `https://github.com/Abhinavkrishna3211/edgeai-predictive-monitor-satellite`
- Reference repo (ground truth for wire format and module conventions): the reference base station's repository (private — ask the maintainer for access)

The goal is not "pick a winner." It is: our satellite firmware must speak the reference base station's wire protocol so it can plug into the reference (unmodified) base station as a node, AND our own gateway (`mic_tools/`) gets rebuilt to mirror the reference module architecture and conventions, while keeping every piece of analytical capability we've already built that the reference gateway doesn't have. The end state is two professional, structurally-identical codebases, each strong where the other is weak, that can interoperate at the wire level.

## Non-negotiable constraint
We can modify our own repo (firmware + gateway) freely. We do NOT modify `edgeai-predictive-monitor` (the reference repository) at all — clone it read-only, purely as a reference to read from and test against. Any change we think the maintainer should make gets written up as a suggestion, never committed to the reference tree.
...
- `bearing_math.py` — BPFO/BPFI/BSF/FTF bearing-defect-frequency computation with harmonic overlays (the reference gateway has no equivalent of this — it's pure anomaly detection with no fault-type physics)
...
Plan with the higher-capability model (the planning tool), build with the standard build model (the AI coding assistant), checkpoint every phase, fresh session per saturated context, the planning tool owns cross-phase state, small diffs and low token-per-turn output on the build side.

1. Planning pass (the higher-capability model / the planning tool, no code): at the start of each phase, re-read the relevant live source in both repos (never trust an earlier session's memory of it), confirm or correct Part D's contract table for anything that phase touches, produce a concrete file-level task list with an explicit exit test, saved as `docs/PHASE_N_PROMPT.md`.
2. Build pass (the standard build model, the AI coding assistant, terse, low-token-per-turn): hand it `docs/PHASE_N_PROMPT.md`. Work in small diffs, not restate unchanged code, skip prose explanations in favor of a one-line rationale per change, stop at the phase's stated exit test rather than opportunistically doing more.
...
**Context change from Abhi:** the reference base station is not just code anymore — it's a finished, physical, running device, and it will not be modified. Our job is now concretely "make our XIAO satellite join the reference base station," with WiFi/MQTT connectivity as the immediate, highest-priority unknown — everything else (DSP correctness, gateway rebuild) is worth less until we know the satellite can actually join the reference network and broker.
...
**Exit:** our physical XIAO satellite appears in the reference base station's unmodified, physically running Uno Q dashboard over real WiFi/MQTT, publishing a wire-valid frame. This unblocks confident execution of Phase 1-3 afterward with the hardest unknown (does WiFi/MQTT to the reference device even work) already answered.
...
Copy the reference repository's `telemetry_schema.json` into `schema/`. Port its `gen_telemetry_schema.py` to emit BOTH `components/epm_codec/telemetry_schema.h` (firmware) AND `gateway/common/telemetry_schema.py` (our gateway), from the one JSON, with its exact "GENERATED ... DO NOT EDIT BY HAND" banner on both outputs.
...
**Exit:** our ESP32 publishes to the Phase 0 local Mosquitto rig and appears on the reference base station's unmodified dashboard with a correct live spectrum. **This is the core project deliverable.**

### Phase 8 — Gateway: extract and restructure `mic_tools/`
Using Part C.2's target layout, extract `recv_verify.py`'s ~15 responsibilities into `gateway/ingestion, pipeline, registry, api, common`. Move every existing capability without loss: bearing_math, RUL, ADWIN, HST, Bayesian fusion, autoencoder inference, adaptive control loop, per-satellite persistence, alerting (webhook/email), dashboard, report generation, Uno Q LED control. Reuse the reference implementation's `common/telemetry_frame.py` decoder (ported in Phase 5) as the single frame-decode path rather than writing a second decoder. Add or migrate a pytest module per extracted package. The Phase 1 gateway tests must all still pass at the end of this phase, now against the restructured code.
**Exit:** Phase 1 gateway tests pass unmodified against the new structure; our gateway and the reference implementation's unmodified gateway can both run as separate subscribers on the same broker and both correctly process frames from either satellite (reference and ours).

### Phase 9 — Gateway: real KX134 driver on our firmware
Port the reference repository's `satellite/src/drivers/kx134.cpp` as the reference implementation, reimplemented behind our `components/epm_hal/include/hal/hal_accel.h` contract as `components/epm_drivers/accel_kx134_spi.c`. Keep `accel_stub.c` selectable via Kconfig for hardware-free development. Follow the register sequence and bring-up order exactly as documented in its `docs/KX134_Interface_Appendix.md` and `docs/SATELLITE_BRINGUP_GUIDE.md` — do not skip bring-up steps.
...
Firmware: `components/epm_dsp/envelope.c` (band-pass → rectify → low-pass → decimate → FFT), exposed as a new schema channel. Coordinate the schema addition with the reference repository's maintainer before implementing, since it touches the shared JSON.
...
- Zero AI/tool attribution anywhere — no co-author trailers naming an AI tool, no "generated with," no mention in code, comments, commit messages, or docs. Before every push (this pattern intentionally matches the literal strings this policy forbids, so it can detect any of them appearing anywhere): `git log --format='%B' | grep -iE 'claude|generated with|co-authored'` must return nothing.
...
## RECONSTRUCTION NOTE (2026-08-06)

Everything below this point (the Phase tracker onward) was **lost and rebuilt**. Root cause: this file was committed to git exactly once, at project start (`3c8346c`, 2026-07-30), and never again — every update since then only ever existed as an uncommitted working-tree edit, per the standing rule that the planning tool alone maintains this file and never runs git commands. A `git filter-repo --force` history rewrite (2026-08-06, cleaning pre-effort AI-attribution trailers, user-authorized) reset the working tree to match the last real commit, silently discarding months of uncommitted updates — this file included. No git-level recovery was possible (checked reflog, full commit history for this path, filter-repo's own metadata — nothing).

This reconstruction is built from the planning tool's own conversation record across the whole effort, not from a git diff, so treat specific numbers/hashes below as high-confidence but not guaranteed byte-exact versus what was lost. **Process fix going forward**: this file should be committed to git periodically (as a docs-only commit, content unchanged by the AI coding assistant, just checked in) so a future working-tree reset can't erase it again — flagged as a new Open Item below.

Parts A-J above are unaffected (they were part of the one real commit and matched the working copy throughout).
...
| Phase | Status | Notes |
|---|---|---|
| 0 | **complete** | Local rig (Mosquitto + his unmodified dashboard + `satellite_node_sim.py`) proven; golden frame captured; `tools/devrig/` wrappers built (`devrig.sh`/`devrig.ps1`, WSL-hosted since his dashboard needs Linux venv paths + `ai-edge-litert` has no Windows wheel). |
| 0.5 | **complete** | Minimal spike satellite (WiFi STA + `esp-mqtt` + ported wire codec) appeared on the reference base station's real physical Uno Q dashboard over real WiFi/MQTT — hardest unknown answered early. Superseded by Phase 3/5/6/7's real (non-spike) implementations, per this phase's own ADR. |
| 1 | **complete** | `tests/host/` (native gcc/CMake) + gateway pytest harness both stood up as the regression net before any restructuring. |
| 2 | **complete** | Firmware DSP correctness fixes (window normalization, Welch-overlap hop sizing) verified/fixed with ADRs. |
| 3 | **complete** | Firmware moved into `components/epm_hal|epm_drivers|epm_dsp|epm_codec`, `src/threads/` — pure move, zero behavior change, confirmed byte-identical output. |
| 4 | **complete** | `docs/BASE_STATION_CONTRACT.md` written, fetched live from the reference base station's repository (private — ask the maintainer for access), main branch. Reversed ADR-014's kurtosis convention to excess/Fisher (`ADR-018`, matches the reference implementation's `raw_features.py` exactly). Confirmed peak=signed-max (matches the reference, own ADR later), skewness/std/crest/peak scalars existed on the wire but weren't yet computed by us (closed in Phase 6/10). Confirmed combined `accel` channel is display-only on the reference side, validating our per-axis-only approach. Found `docs/Appendix_B_Wire_Protocol_Specification.md` (reference repository) is stale vs. the actual code — `docs/SENSOR_TELEMETRY_FRAME_PLAN.md` is current. |
| 5 | **complete** | `schema/telemetry_schema.json` copied from his repo, `schema/gen_schema.py` ported, generates both `components/epm_codec/include/frame_codec/telemetry_schema.h` and `gateway/common/telemetry_schema.py` from one JSON with the `GENERATED ... DO NOT EDIT BY HAND` banner. |
| 6 | **complete** | Fuser/section-list encoder implemented; frames pass his own `telemetry_frame_test.py` directly. All 6 scalars (rms/kurtosis/std/peak/crest_factor/skewness) computed for all 4 groups (mic/x/y/z) — closed Phase 4's flagged gap. |
| 7 | **complete** (7a/7b/7c) | `link_mqtt.c` behind `hal_transport.h` via `esp-mqtt`; TCP+AES path retired (`ADR-023` supersedes earlier transport ADRs, not deleted). **Core project deliverable met**: our XIAO satellite appeared on the reference base station's unmodified dashboard with a correct live spectrum, confirmed end-to-end multiple times (`ADR-026`'s live broker-connectivity breakthrough; real `STATUS_LED` remote command delivery confirmed on physical hardware — LED changed to solid orange from a real MQTT command, then BREATHE/STROBE modes confirmed on a follow-up session with `ADR-025`'s disconnect-revert firing correctly). Power management (DFS, light sleep, TX cap) confirmed to survive the transport swap. |
| 8 | **complete** (8a/8b1/8b2/8b3/8c) | `mic_tools/` (originally ~10,000 lines, `recv_verify.py` alone ~4,330) restructured into `gateway/{ingestion,pipeline,registry,api,common}` + `main.py`. `recv_verify.py` deliberately kept alive at 1,439 lines as the shared-state/pipeline core (`ADR-029`) rather than fully retired — every `gateway/*` module lazy-imports it. Real MQTT decode (`gateway/ingestion/mqtt_subscriber.py`) replaced the dead TCP+AES protocol as the primary path; `tcp_legacy.py` kept alive for `tools/satellite_sim.py`'s manual testing (`ADR-028`). **Live cross-gateway exit test run for real**: our gateway and the reference implementation's unmodified gateway both live on the same broker, each correctly processing frames from either satellite (reference and ours) — the phase's stated exit criterion, actually met, not just claimed. One real bug found and fixed along the way: `tools/satellite_sim.py` had a genuine `ModuleNotFoundError` when run from the repo root as its own docstring instructed (only worked by cwd-on-sys.path accident from inside `tools/`) — fixed (`144b0d5`). |
| 9 | **complete** | Real KX134 SPI accelerometer driver (`components/epm_drivers/accel_kx134_spi.c`) ported from his `satellite/src/drivers/kx134.cpp`, behind our `hal_accel.h`. `accel_stub.c` kept selectable via Kconfig (`EPM_ACCEL_USE_STUB`) for hardware-free dev. Hardware-validated: WHO_AM_I correct, FIFO+interrupt path delivers data with zero dropped frames over sustained runs. |
| 10 | **complete** (10a/10b/10c) | Full convention-compliance + hardening pass. Real bugs found and fixed: `imu_task.c` discarding `hal_accel_read_block()`'s return (a disconnected KX134 would go unnoticed), two display drivers not checking `esp_err_t`, 6 of 9 gateway `except Exception:` sites silently swallowing errors, `imu_task_start()` itself discarding `hal_accel_init()`/`hal_accel_start()`'s returns, gateway's `logging.basicConfig()` never being called anywhere. `docs/CONVENTIONS.md` written from scratch (181 lines) — no standalone conventions reference had ever existed on either side despite Part I claiming to be one. **Critical hardware-validated bug found and fixed same day**: KX134 didn't recover from a power-loss without a manual board reset (`hal_accel_init()`/`hal_accel_start()` only ran once at task startup, the `IMU_FAIL_MAX` escalation branch only logged). Fixed (`cb20a8b`) by splitting the register-reprogram tail into a shared `hal_accel_reinit()`, deliberately not re-running the one-time SPI-bus/semaphore/GPIO-ISR setup (the trap explicitly flagged and correctly avoided). Live re-tested: fault detected, reinit fires, recovers without reboot. |
| 11 | **complete** (11a/11b), both hardware/live-verified | **11a**: `components/epm_dsp/envelope.c` — band-pass 2-8kHz (matching `fault_models.py`'s structural-resonance convention, no sensor-specific resonance documented for the KX134 per `ADR-017`) → full-wave rectify → 1kHz low-pass → decimate-by-8 (25,600→3,200Hz) → FFT, landing exactly on `EPM_MODEL_SPECTRUM_BINS` (128 bins). Schema: `accel_x/y/z_envelope` (ids 9-11), `ADR-032` records that no coordination with the reference repository's maintainer was needed (the reference implementation's own schema doc comment + `registry.py`'s confirmed generic-channel fallthrough for anything outside its `SensorChannel` enum). RAM 47.4%→49.5% (+6.9KB); runtime heap margin hardware-confirmed stable at 24-31KB over 6.5 min of live WiFi+MQTT+DSP load (well above `ADR-020`'s old ~7-10KB figure — that old number was measured under a since-retired dual-transport path, exactly as `ADR-020` itself predicted would improve once Phase 7 landed). **11b**: `gateway/ingestion/mqtt_subscriber.py` decodes the 3 envelope channels reading each section's own `fs` off the wire (never hardcoded — proven via a deliberately-odd-fs round-trip test); `gateway/api/live_plot.py` adds 3 envelope panels with BPFO/BPFI/BSF/FTF markers computed against each panel's own decoded fs. Sanity check: a synthesized 6205-bearing outer-race defect at 1500 RPM (BPFO=82.40Hz) puts the real spectral peak at 81.25Hz against an 82.40Hz marker — 0.09 bins apart. |
| 12 (new, not in original plan, added 2026-08-06) | **complete** (12a/12b), both hardware-validated | WiFi/MQTT captive-portal onboarding, ported from a design found live in the reference repository's separate `satellite/` repo (`docs/WIFI_ONBOARDING_PLAN.md`, `hal_provisioning.h`) after the user surfaced a screenshot referencing it. **12a**: NVS-backed credential storage (`net_credentials.c`), 5-state provisioning state machine (`BOOT_STA_ATTEMPT→PROVISIONING→STA_TESTING→CONNECTED→RECOVERING`, `wifi_provision_task.c`), `hal_provisioning.h` contract. Deliberate improvement over the reference design: `ADR-031` — per-device `esp_fill_random()`-generated, NVS-persisted WPA2 password instead of the reference's open/no-password AP (the reference implementation's own admitted acceptable-for-demo-scale tradeoff). **12b**: real captive portal — `provisioning.c` orchestration, `ap_credentials.c` (the ADR-031 password), `dns_captive.c` (raw UDP-53 wildcard responder, ESP-IDF's own `captive_portal` example pattern), `http_captive.c` (`esp_http_server` form + OS captive-portal probe redirects), shared `node_id.c` (extracted from `link_mqtt.c`'s prior duplicate). **Full pipeline hardware-validated end to end by the user** (~152s continuous run, zero errors across every subsystem — WiFi 1 connect/0 disconnects, MQTT 700 frames/0 failures, mic/DSP/IMU/accel/heap/LED all clean). **3 real bugs found and fixed during that live session, all committed** (`86ab5c5`/`03d2c22`/`0173f98`): MQTT silently never starting after a heap-margin race with the portal's teardown (fixed with a 2s-backoff retry loop), the AP tearing down before a phone's success-page poll could land (fixed with a 5s grace delay), a stale dev-bench broker IP. Two non-bugs correctly ruled out without firmware changes (phone OS auto-switching off a no-internet network; mistyped AP passwords). **Flash-size false alarm found and corrected**: Phase 12b's own "98.7% flash used, 13KB headroom" was a PlatformIO reporting artifact (`board_build.partitions` was never set in `platformio.ini`, so the percentage was computed against a stale 1MB board-manifest default instead of the real ~7.27MB custom partition table) — fixed (`2d77e63`), real usage is 14.2%/~6.1MB headroom, and this retroactively corrects every prior phase's flash-percentage tracking too (the underlying byte counts were always fine, only the percentage/headroom framing was wrong the whole time). |

---

---

## Open items (the planning tool maintains this — check at the start of any new session)

- **This file's own history — CLOSED, root-caused, process fix pending (2026-08-06):** see the RECONSTRUCTION NOTE at the top of this section. This file must be committed to git periodically going forward (docs-only commit, the planning tool provides the content, the AI coding assistant just runs `git add docs/MASTER_PLAN.md && git commit` with no content changes) so a future working-tree reset (rebase, filter-repo, hard reset, etc.) can't silently erase it again. Not yet scheduled to a phase — flag for the next session.
- **LED `RGB_TRIPPED` mismatch — found 2026-08-06 via the comparison audit, not yet fixed.** See the audit section above. Small, self-contained fix: `display_neopixel.c`'s `RGB_TRIPPED` → `0xFF0000`/STROBE/1000ms, correct the two stale "no NodeStatus analog" comments (this file's own prior text and `ADR-016`'s decision table both need the correction too, next time either is touched), decide + document `PAUSED`/`OFFLINE`/`IDLE`'s scope status one way or the other.
- **Pre-existing AI-tool co-author trailer lines in commit history, predating 2026-07-30 — CLOSED (2026-08-06).** 13 commits cleaned via `git filter-repo` (user-authorized, `--force` flag used since the repo had uncommitted changes — see this file's own RECONSTRUCTION NOTE for the fallout). Backup pushed to `origin` before the rewrite (`backup-pre-attribution-rewrite-2026-08-06`, later also fixed locally after its pointer got swept into the same rewrite — origin's copy was always correct). Tree-content verified byte-identical pre/post rewrite. `main` fast-forwarded on `origin` (`54a60d7→cf196a7`), `feat/base-station-interop` pushed fresh (never existed on `origin` before). Zero real trailers remain on either branch, independently verified.
- **`devrig.sh`/`devrig.ps1` broker-host override — CLOSED (2026-08-06).** His `base-station/start_desktop_dashboard.sh` hardcodes `MQTT_HOST=localhost` with no override; our wrapper now accepts `--mqtt-host`/`--mqtt-port` (CLI flag or `MQTT_HOST`/`MQTT_PORT` env var) and bypasses his wrapper (invoking `base-station/python/main.py` directly) when a non-localhost host is given. Default path unchanged.
- **`EPM_MQTT_BROKER_HOST` default (`10.42.0.1`) unverified against the real Uno Q** — carried since Phase 0.5, overridable via a `platformio.ini` build_flag without touching the header, never actually blocked a session. Still flagged, not scheduled.
- **Phase 3.5-era latent esp-mqtt bug** (`esp_mqtt_client_init()` never checks `esp_event_loop_create()`'s return; under tight heap the allocation fails and the next call null-derefs) — root-caused via `addr2line`, documented, but the underlying vendored `mqtt_client.c` bug itself was never patched (worked around via `ADR-024`'s heap-margin guard + Phase 12b's retry-loop fix instead). Low priority given the workarounds hold.
- **NeoPixel physical color rendering** — RMT/driver logic confirmed correct on hardware multiple times over the project; exact color-accuracy (gamma/channel-order) was checked by direct visual comparison in at least one session and found matching, but no camera-based objective verification has ever been done. Low priority.
- **11b's layout** (5-row×6-col grid with 7 spectrum panels) — functional and tested, but getting visually dense; worth a UX pass if this dashboard becomes more than a debug tool.
