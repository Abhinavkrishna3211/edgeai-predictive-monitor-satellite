...
| 0.5 | **complete** | Minimal spike satellite (WiFi STA + `esp-mqtt` + ported wire codec) appeared on the reference-repo maintainer's real physical Uno Q dashboard over real WiFi/MQTT — hardest unknown answered early. Superseded by Phase 3/5/6/7's real (non-spike) implementations, per this phase's own ADR. |
...
| 4 | **complete** | `docs/BASE_STATION_CONTRACT.md` written, fetched live from `github.com/rahuljeyaraj/edgeai-predictive-monitor@main`. Reversed ADR-014's kurtosis convention to excess/Fisher (`ADR-018`, matches his `raw_features.py` exactly). Confirmed peak=signed-max (matches his, own ADR later), skewness/std/crest/peak scalars existed on the wire but weren't yet computed by us (closed in Phase 6/10). Confirmed combined `accel` channel is display-only on his side, validating our per-axis-only approach. Found `docs/Appendix_B_Wire_Protocol_Specification.md` (his repo) is stale vs. the actual code — `docs/SENSOR_TELEMETRY_FRAME_PLAN.md` is current. |
...
| 11 | **complete** (11a/11b), both hardware/live-verified | **11a**: `components/epm_dsp/envelope.c` — band-pass 2-8kHz (matching `fault_models.py`'s structural-resonance convention, no sensor-specific resonance documented for the KX134 per `ADR-017`) → full-wave rectify → 1kHz low-pass → decimate-by-8 (25,600→3,200Hz) → FFT, landing exactly on `EPM_MODEL_SPECTRUM_BINS` (128 bins). Schema: `accel_x/y/z_envelope` (ids 9-11), `ADR-032` records that no coordination with the reference-repo maintainer was needed (his own schema doc comment + `registry.py`'s confirmed generic-channel fallthrough for anything outside his `SensorChannel` enum). RAM 47.4%→49.5% (+6.9KB); runtime heap margin hardware-confirmed stable at 24-31KB over 6.5 min of live WiFi+MQTT+DSP load (well above `ADR-020`'s old ~7-10KB figure — that old number was measured under a since-retired dual-transport path, exactly as `ADR-020` itself predicted would improve once Phase 7 landed). **11b**: `gateway/ingestion/mqtt_subscriber.py` decodes the 3 envelope channels reading each section's own `fs` off the wire (never hardcoded — proven via a deliberately-odd-fs round-trip test); `gateway/api/live_plot.py` adds 3 envelope panels with BPFO/BPFI/BSF/FTF markers computed against each panel's own decoded fs. Sanity check: a synthesized 6205-bearing outer-race defect at 1500 RPM (BPFO=82.40Hz) puts the real spectral peak at 81.25Hz against an 82.40Hz marker — 0.09 bins apart. |
| 12 (new, not in original plan, added 2026-08-06) | **complete** (12a/12b), both hardware-validated | WiFi/MQTT captive-portal onboarding, ported from a design found live in the reference-repo maintainer's separate `satellite/` repo (`docs/WIFI_ONBOARDING_PLAN.md`, `hal_provisioning.h`) after the user surfaced a screenshot referencing it. **12a**: NVS-backed credential storage (`net_credentials.c`), 5-state provisioning state machine (`BOOT_STA_ATTEMPT→PROVISIONING→STA_TESTING→CONNECTED→RECOVERING`, `wifi_provision_task.c`), `hal_provisioning.h` contract. Deliberate improvement over his design: `ADR-031` — per-device `esp_fill_random()`-generated, NVS-persisted WPA2 password instead of his open/no-password AP (his own admitted acceptable-for-demo-scale tradeoff). **12b**: real captive portal — `provisioning.c` orchestration, `ap_credentials.c` (the ADR-031 password), `dns_captive.c` (raw UDP-53 wildcard responder, ESP-IDF's own `captive_portal` example pattern), `http_captive.c` (`esp_http_server` form + OS captive-portal probe redirects), shared `node_id.c` (extracted from `link_mqtt.c`'s prior duplicate). **Full pipeline hardware-validated end to end by the user** (~152s continuous run, zero errors across every subsystem — WiFi 1 connect/0 disconnects, MQTT 700 frames/0 failures, mic/DSP/IMU/accel/heap/LED all clean). **3 real bugs found and fixed during that live session, all committed** (`86ab5c5`/`03d2c22`/`0173f98`): MQTT silently never starting after a heap-margin race with the portal's teardown (fixed with a 2s-backoff retry loop), the AP tearing down before a phone's success-page poll could land (fixed with a 5s grace delay), a stale dev-bench broker IP. Two non-bugs correctly ruled out without firmware changes (phone OS auto-switching off a no-internet network; mistyped AP passwords). **Flash-size false alarm found and corrected**: Phase 12b's own "98.7% flash used, 13KB headroom" was a PlatformIO reporting artifact (`board_build.partitions` was never set in `platformio.ini`, so the percentage was computed against a stale 1MB board-manifest default instead of the real ~7.27MB custom partition table) — fixed (`2d77e63`), real usage is 14.2%/~6.1MB headroom, and this retroactively corrects every prior phase's flash-percentage tracking too (the underlying byte counts were always fine, only the percentage/headroom framing was wrong the whole time). |

---

## Comparison audit against the reference-repo maintainer's repo (2026-08-06, `docs/RAHUL_COMPARISON_AUDIT.md`)

The user's original standing request ("compare the reference-repo maintainer's code and ours, check for any issue or mismatch") — done via live research directly against his current repo, not delegated. Full findings in that file; summary: wire protocol/schema, DSP scalar formulas (all 6), firmware naming/error-handling conventions all **MATCH**. WiFi provisioning security model, gateway feature scope (his is a full commissioning/protection dashboard, ours is intentionally narrower), and git/ADR/attribution hygiene are **intentional, reasoned divergences**, nothing to fix. **One real, previously-mislabeled bug found**: `display_neopixel.c`'s `RGB_TRIPPED` (`0xFF00FF`/STROBE/150ms) doesn't match his `status_color.py` (`NodeStatus.TRIPPED` = `#ff0000`/STROBE/1000ms, his own comment explains 1000ms as deliberately reading "latched, already acted"). Worse than a simple mismatch: our TRIPPED magenta is *exactly* his `IDLE` color — a real semantic collision if our satellite and his base-station-driven nodes are ever viewed side by side. Both `ADR-016`'s decision table and `display_neopixel.c`'s own comment claiming TRIPPED "has no NodeStatus analog" are factually wrong. Not yet fixed (research-only pass) — small, self-contained follow-up recommended. `PAUSED`/`OFFLINE`/`IDLE` also have zero local representation — may be a legitimate satellite-side scope boundary, but currently undocumented as a deliberate one.
