...
`docs/MASTER_PLAN.md` Part G, Phase 2 flagged the Welch-overlap implementation
in `src/dsp_task.c` as "verify, don't assume" — no Phase 1 host test exists
...
comment at the accumulation site in `dsp_task.c` and here for Phase 4 (or
whichever phase next touches `epm_protocol.h`'s comments) to reconcile.
...
- No test in `tests/host/` exercises this path (same linking limitation
  that left it untested in Phase 1); regression coverage for this fix is
