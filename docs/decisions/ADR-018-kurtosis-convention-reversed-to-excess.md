...
ADR-014 (Phase 2) chose RAW/Pearson kurtosis (Gaussian ≈ 3.0) to match our own
...
excess/Fisher convention documented in `docs/MASTER_PLAN.md` Part D was
...
(Gaussian ≈ 0.0), matching `docs/MASTER_PLAN.md` Part D's original draft, not
ADR-014's RAW/Pearson choice.

This ADR does not itself change code (Phase 4 is planning-only, no firmware/gateway
edits). Required follow-up, for whichever phase next touches scalar computation:
...
- Not superseded/deleted per Part I — ADR-014 stays in the tree, marked
...
- No urgency to land the code fix immediately — no real kurtosis is on the wire
  yet (satellite still publishes synthetic frames, Phase 6 is where real DSP
  output starts flowing). Recommend folding into Phase 6's scope (it already
  touches the fuser/encoder path) rather than a standalone emergency fix.
