...
**Cons:** Directly contradicts Part B (already decided) and defeats the entire purpose of this phase — a synthetic sine-wave stub cannot detect real bearing faults, and interop testing against the reference's real sensor data is impossible with synthetic input.
...
**Cons:** Would require changing `hal_accel.h` (out of scope — Task 2 explicitly states that header is "already pure C, already correct... no changes needed there") and would break `accel_stub.c` and every existing caller in `src/threads/imu_task.c`, which already consumes per-axis float and has no reason to change (Part B: "reimplement behind our `hal_accel.h`", not the other way around).
...
## Validation
Compile-verified only in this session (`pio run -e xiao_esp32s3`) — see the session's final report for whether physical KX134 hardware was available to run the live exit test (WHO_AM_I, at-rest g-values, zero-dropped-frame sustained run). `tests/host/` does not cover this driver (ESP-IDF/SPI-dependent, not host-testable).
