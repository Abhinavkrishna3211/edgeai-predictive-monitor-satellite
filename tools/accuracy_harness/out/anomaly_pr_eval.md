...
Healthy: 300 training + 80 held-out test samples (jittered Normal-zone tuples, clipped below K_WARN/CREST_WARN). Anomalous: 120 samples (15 severity-jittered deep-inside variants per non-Normal label x 8 labels), generated by `synth_frames.py` and scored via a synthetic mic FFT that reproduces each sample's hi_r/lo_r/mid_r exactly. No hardware/MQTT involved.
